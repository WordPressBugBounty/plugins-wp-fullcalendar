...
class WP_FullCalendar {
	...
	public static function ajax() {
		...
		$args['has_password']   = false; //defense in depth: don't list password-protected posts (or hand out nonces for them) in a public calendar. Filterable below via wpfc_fullcalendar_args.
	...
	public static function qtip_content() {
		...
		if ( ! empty( $_REQUEST['post_id'] ) && ! empty( $_REQUEST['nonce'] ) && wp_verify_nonce( $_REQUEST['nonce'], 'wpfc_item_nonce_' . $_REQUEST['post_id'] ) ) {
			...
			// The nonce only proves the item was listed in the calendar, not that the viewer may read it; is_post_publicly_viewable() needs WP 5.7, so older sites rely on the password check alone.
			if ( $post && ( ! function_exists( 'is_post_publicly_viewable' ) || is_post_publicly_viewable( $post ) ) && ! post_password_required( $post ) ) {
			...
			foreach ( get_object_taxonomies( $post_type ) as $taxonomy_name ) {
				...
				if ( count( get_terms( $taxonomy_name, array( 'hide_empty' => 1 ) ) ) > 0 && in_array( $taxonomy_name, $search_taxonomies ) ) {
					...
					$taxonomy_args = array(
						...
						'name'            => $taxonomy_name,
						'hierarchical'    => true,
						'class'           => 'wpfc-taxonomy ' . $taxonomy_name,
						'taxonomy'        => $taxonomy_name,
	...
	public static function footer_js() {
		...
		<?php
			...
			$file_long   = __DIR__ . '/includes/js/locale/' . $locale_code . '.js';
			$file_short  = __DIR__ . '/includes/js/locale/' . substr( $locale_code, 0, 2 ) . '.js';
